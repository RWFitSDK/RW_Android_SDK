...
>
> 设备只保存最近一次约1分钟的PPG原始数据, 无论由定时监测还是手动测量触发, 下一次采集都可能覆盖尚未同步的数据. 每次采集完成后, 设备会通过 `SensorRawControlCallback.onResult(reason)` 通知停止; 收到通知后应及时调用 `ringGetHistorySensorRaw()` 同步并持久化数据.
>
